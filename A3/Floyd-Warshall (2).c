...
# define MASTER 0
// make sure you pu n into metrix [n][n]
# define n 10
//
...
int main(int argc, char *argv[])
{
    ...
    // int sub_x_ranks[num_processors_in_d];
    // MPI_Allgather(rank, 1, MPI_INT, sub_x_ranks, 1, MPI_INT, x_com);

    // int sub_y_ranks[num_processors_in_d];
    // MPI_Allgather(rank, 1, MPI_INT, sub_y_ranks, 1, MPI_INT, y_com);

    // printf("rank :%d , cord:%d\n", rank, cord[0]);
    MPI_Group group_x;
    MPI_Comm_group(x_com, &group_x);
    int sub_x_rank;
    MPI_Group_rank(group_x, &sub_x_rank);

    MPI_Group group_y;
    MPI_Comm_group(y_com, &group_y);
    int sub_y_rank;
    MPI_Group_rank(group_y, &sub_y_rank);
   printf("rank :%d, xR:%d,  yR:%d\n", rank, sub_x_rank,sub_y_rank);
...
//         }
    ...
    // int cord[3] ;
    // MPI_Cart_coords (x_com, rank, 2 , cord);
    // printf("rank :%d , cord:%d\n", rank, cord[0]);
      ...
      //\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\|||||||||||||||||||||||||||||     
       ...
       if (rank == 0)
       {
        ...
        if (sub_x_rank == p_index)
        {
           
            int left_p;
            int right_p;
            MPI_Cart_shift (com_cart, 1, 1, &left_p, &right_p);
             if (left_p !=-1)
             {
                MPI_Send(&col_val,  1, MPI_INT, left_p, 37, com_cart);
 
             }

            if (right_p !=-1)
             {
                MPI_Send(&col_val,  n/num_processors_in_d, MPI_INT, right_p, 37, com_cart);
        ...
        // if not  acrual sender
        if (sub_x_rank != p_index)
        {
            MPI_Status stat;
        
            MPI_Recv(&col_val, n/num_processors_in_d, MPI_INT, MPI_ANY_SOURCE, 37, com_cart, &stat);

          //  printf(" p%d  got from%d   or %d \n", rank, recSize , stat.MPI_SOURCE  );
            ...
            if (stat.MPI_SOURCE  == right_p)
            {
                if (left_p !=-1)
                    {
                    MPI_Send(&col_val,  n/num_processors_in_d, MPI_INT, left_p, 37, com_cart);
    ...
    //    }
         ...
         //int recSize  = rank;
        if (sub_y_rank == p_index)
        {
           
            int up_p;
            int down_p;
            MPI_Cart_shift (com_cart, 0, 1, &up_p, &down_p);
             if (up_p !=-1)
             {
                MPI_Send(&row_val,  1, MPI_INT, up_p, 72, com_cart);
 
             }

            if (down_p !=-1)
             {
                MPI_Send(&row_val,  n/num_processors_in_d, MPI_INT, down_p, 72, com_cart);
        ...
        if (sub_y_rank != p_index)
        {
            ...
            MPI_Recv(&row_val, n/num_processors_in_d, MPI_INT, MPI_ANY_SOURCE, 72, com_cart, &stat);
           ...
           // printf(" p%d  got from%d   or %d \n", rank, recSize , stat.MPI_SOURCE  );
            ...
            if (stat.MPI_SOURCE  == down_p)
            {
                if (up_p !=-1)
                    {
                    MPI_Send(&row_val,  n/num_processors_in_d, MPI_INT, up_p, 72, com_cart);
